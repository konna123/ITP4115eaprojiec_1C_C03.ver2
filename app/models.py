...
# Existing tables (Gender, Country, Department, Function, Benefit, EmployeeHistory, Employee, MenuItem, MenuCategory, News, NewsCategory)
# ...

# New database table: JobPost
class JobPost(Model):
    __tablename__ = 'job_post'
    id = Column(Integer, primary_key=True)
    title = Column(String(100), nullable=False)
    description = Column(Text, nullable=False)
    date_posted = Column(Date, default=datetime.date.today(), nullable=True)
    jobpost_category_id = Column(Integer, ForeignKey('jobpost_category.id'), nullable=False)
    jobpost_category = relationship("JobPostCategory")

# New database table: JobPostCategory
class JobPostCategory(Model):
    __tablename__ = 'jobpost_category'
    id = Column(Integer, primary_key=True)
    name = Column(String(50), nullable=False)

# New database table: User
class User(Model):
    __tablename__ = 'user'
    id = Column(Integer, primary_key=True)
